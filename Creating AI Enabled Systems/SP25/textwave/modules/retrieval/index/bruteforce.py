...
class FaissBruteForce:
    ...
    def search(self, query, k=5):
        ...
        Parameters
        ----------
        query : 1‑D or 2‑D array‑like
            The query vector(s).  If 1‑D, shape will be coerced to (1, dim).
        k : int
            Number of neighbours to retrieve.

        Returns
        -------
        distances : np.ndarray  shape (n_query, k)
        indices    : np.ndarray  shape (n_query, k)
            FAISS returns indices relative to the order in which the vectors
            were added, so you can map them back to metadata with
        ...
        if q.ndim == 1:
            q = q.reshape(1, -1)
        if self.metric == "cosine":
            faiss.normalize_L2(q)          # make inner‑product == cosine
