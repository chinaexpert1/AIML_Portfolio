...
class RawDataHandler:
    ...
    def save_log(self, log_type='log'):
        """
        Saves the global log_messages to a JSON file.
        """
        ...
        with open(save_filepath, 'w') as f:
            json.dump({"logs": log_messages}, f, indent=4)
        
        print(f"Logs saved to: {save_filepath}")
    ...
    def transform(self, customer_information: pd.DataFrame, 
        ...
        log("New dataset shape:", merged_data.shape)
        ...
        # log the count of NaN values for each column
        ...
        log(nan_counts[nan_counts > 0])

        # Drop rows where any NaN values are present
        cleaned_data = cleaned_data.dropna()
        log(f"Dropped rows with missing values. New dataset shape: {cleaned_data.shape}")
        ...
        print(f"Total rows dropped due to missing values: {total_dropped}")
        print("Rows dropped by fraud_label:")
        for label, count in dropped_by_fraud.items():
            print(f"  {label}: {count}")
        print("Missing values by column (before dropping rows):")
    ...
    def save_cleaned_data(self, df: pd.DataFrame, partition_type):
        """
        ...
        """
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"dataset_{partition_type}_{timestamp}.csv"
        save_filepath = os.path.join(self.save_path, filename)
        os.makedirs(self.save_path, exist_ok=True)
