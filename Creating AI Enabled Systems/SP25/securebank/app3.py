...
def train_model():
    ...
    def evaluate_fraud_models(X_train, X_val, X_test, y_train, y_val, y_test, 
        ...
        log(f"  F1 Score:  {test_default_f1:.3f}")
        ...
        log(f"  F1 Score:  {test_optimal_f1:.3f}")
        ...
        test_summary = [
        f"Optimal threshold for {model}: {best_threshold:.3f} (maximizing recall with ≥30% precision)"
        "At this threshold - Precision: {:.3f}, Recall: {:.3f}, F1: {:.3f}".format(best_precision, best_recall, best_f1),
        "=== Test Set Results ===",
        "[Default Threshold]",
        "  Precision: {:.3f}".format(test_default_precision),
        "  Recall:    {:.3f}".format(test_default_recall),
        "  F1 Score:  {:.3f}".format(test_default_f1),
        "[Optimal Threshold]",
        "  Threshold: {:.3f}".format(best_threshold),
        "  Precision: {:.3f}".format(test_optimal_precision),
        "  Recall:    {:.3f}".format(test_optimal_recall),
        "  F1 Score:  {:.3f}".format(test_optimal_f1),
        "[Overall Scores]",
        "  ROC AUC:   {:.3f}".format(test_auc),
        "  PR AUC:    {:.3f}".format(test_avg_precision)
    ]
