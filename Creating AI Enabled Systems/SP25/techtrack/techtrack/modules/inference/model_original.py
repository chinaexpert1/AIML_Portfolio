...
class Detector:
    ...
    def predict(self, preprocessed_frame: np.ndarray) -> List[np.ndarray]:
        ...
        **Reference:**
        ...
        :return: A single NumPy array containing all detections.
        """
        ...
        blob = cv2.dnn.blobFromImage(preprocessed_frame, scalefactor=1/255.0, size=(416, 416), swapRB=True, crop=False)
        ...
        # Flatten all detections into a single NumPy array
        predictions = np.concatenate(outputs, axis=0)

        # print(f"[INFO] YOLO Prediction Successful! Output shape: {predictions.shape}")
        return predictions
