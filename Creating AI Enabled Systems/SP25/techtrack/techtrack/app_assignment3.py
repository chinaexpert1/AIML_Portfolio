    ...
    def run(self) -> None:
        ...
        **Example Class Usage:**
        ...
        for frame in self.stream.capture_video():
            ...
            bboxes, class_ids, confidence_scores, class_scores = self.nms.filter(bboxes, class_ids, confidence_scores, class_scores)
            
            for bbox, class_id, score in zip(bboxes, class_ids, confidence_scores):
                print(f"Frame {frame_count}: Class {class_id}, Score: {score:.2f}, BBox: {bbox}")
            
            processed_frame = self.draw_boxes(frame, bboxes, class_ids, confidence_scores)
            self.save_frame(processed_frame, frame_count)
            frame_count += 1
...
if __name__ == "__main__":
    ...
    service = InferenceService(stream, model, nms, save_dir=current_directory)
