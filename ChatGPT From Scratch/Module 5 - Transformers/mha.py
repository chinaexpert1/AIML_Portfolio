# mha.py by Andrew Taylor (with causal mask)
...
class CustomMHA(torch.nn.Module):
	...
	def forward(self, x):
		...
		# 
		# Create a lower triangular matrix of shape (S, S)
		# where positions can onnly attend to previous positions (and itself)
		# torch.tril with 1s on/below diagonal, 0s above
		causal_mask = torch.tril(torch.ones(S, S, device=x.device))
		
		# Convert to boolean: True where we want to keep values, False where we mask
		# We want to mask out the upper triangle are future positions
		causal_mask = causal_mask == 0  # refine: True for where positions to mask (upper triangle)

		# 
		# apply causal mask
		# 
		# Use masked_fill to set masked positions to -inf before softmax
		# This ensures masked position s have attention weight of 0 after softmax
